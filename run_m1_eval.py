...
RAW_PATH    = os.path.join(RESULTS_DIR, "m1_raw.json")
BASELINE    = os.path.join(RESULTS_DIR, "m1_baseline.json")
SLEEP_SEC   = 3.0   # between LLM calls (Groq free tier: 30 RPM = min 2s/req)
N_EVAL      = None  # None = all 400; set to int to limit
N_RAGAS     = 50    # samples for RAGAS (each sample = multiple LLM calls)
...
def main():
    ...
    for item in tqdm(todo, desc="M1 Eval", file=sys.stderr):
        try:
            ...
            # Save inside try: ensures checkpoint runs even after exceptions
    ...
    print(f"\n{'='*45}")
    print(f"M1 Naive RAG — Local Metrics (n={local['n_samples']})")
    print(f"{'='*45}")

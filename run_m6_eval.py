...
RAW_PATH  = os.path.join(RESULTS_DIR, "m6_raw.json")
BASELINE  = os.path.join(RESULTS_DIR, "m6_baseline.json")
SLEEP_SEC = 3.0
N_EVAL    = None  # None = tất cả
N_RAGAS   = 50
SAVE_EVERY = 10
...
def main():
    ...
    for item in tqdm(todo, desc="M6 Eval", file=sys.stderr):
        try:
    ...
    print(f"\n{'='*50}")
    print(f"M6 Advanced RAG (Hybrid + Reranker) — n={local['n_samples']}")
    print(f"{'='*50}")
    print(f"  Exact Match       : {local['exact_match']:.4f}")
    print(f"  ROUGE-L           : {local['rouge_l']:.4f}")
    if "context_hit_rate" in local:
        print(f"  Context Hit Rate  : {local['context_hit_rate']:.4f}")

    print(f"\nRunning RAGAS on {min(N_RAGAS, len(results))} samples...", flush=True)
    ...
    if ragas:
        ...
        print("  RAGAS skipped (check Groq quota)")

    baseline = {
        "method": "M6_AdvancedRAG",
        "config": {
            "top_k": 4,
            "initial_k": 12,
            "chunk_size": 400,
            ...
            "retrieval": "BM25 + Dense + RRF + Multilingual CrossEncoder Reranker",
            "reranker": "nreimers/mmarco-mMiniLMv2-L12-H384-v1",
        },
        "metrics": {**local, **ragas},
    }

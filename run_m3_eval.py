...
RAW_PATH   = os.path.join(RESULTS_DIR, "m3_raw.json")
BASELINE   = os.path.join(RESULTS_DIR, "m3_baseline.json")
SLEEP_SEC  = 5.0   # M3 makes 2 LLM calls per item (expand + generate)
N_EVAL     = None
N_RAGAS    = 50
SAVE_EVERY = 10
...
def main():
    ...
    for item in tqdm(todo, desc="M3 Eval", file=sys.stderr):
        try:
            ...
            results.append({
                ...
                "query_variants": out.get("query_variants", []),
            })
    ...
    print(f"\n{'='*45}")
    print(f"M3 RAG-Fusion — Local Metrics (n={local['n_samples']})")
    print(f"{'='*45}")
    ...
    baseline = {
        ...
        "config": {
            ...
            "retrieval": "Multi-query + BM25 + Dense + RRF×2",
            "n_variants": 3,
        },

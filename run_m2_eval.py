...
RAW_PATH   = os.path.join(RESULTS_DIR, "m2_raw.json")
BASELINE   = os.path.join(RESULTS_DIR, "m2_baseline.json")
SLEEP_SEC  = 3.0
N_EVAL     = None
N_RAGAS    = 50
SAVE_EVERY = 10
...
def main():
    ...
    for item in tqdm(todo, desc="M2 Eval", file=sys.stderr):
        try:
    ...
    print(f"\n{'='*45}")
    print(f"M2 Hybrid RAG — Local Metrics (n={local['n_samples']})")
    print(f"{'='*45}")
